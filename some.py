...
def result(user_answers, data_file):
    ...
    for x in range(0, len(winners)):
        for y in range(0, len(rankings)):
            if winners[x] == rankings[y] and data_file['alt'][y] not in winners_alt:
                winners_alt.append(data_file['alt'][y])
                break
            elif winners[x] == rankings[y] and data_file['alt'][y] in winners_alt:
                winners_alt.append(data_file['alt'][y + 1])
